...
- 必須の folder が欠けていても、現存する source の意味監査を止める理由にはしない。たとえば worker の配置が不正でも、その source を concerns の並行処理規律と languages の言語 ecosystem の実現規律へ照合する。対象 artifact 自体が存在しない確認だけを実行不能にし、folder 欠落を理由に領域全体を見送らない。
