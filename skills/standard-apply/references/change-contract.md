...
受入条件に completion、success、failure、state など意味と authority を要する語があれば、依頼文や対象 source の局所的な読みだけで意味を固定しない。同じ語または型を手掛かりに、対象 project の宣言された decision record、state authority、writer、caller、既存 test を、source から実演された link に沿って material evidence が揃うまで追い、変更契約と検証へ反映する。caller を一つ読んだだけで追跡を止めず、authority または既存 test を確認できなければ、その語の意味と検証条件を未確認にする。future または task の終了と、domain の terminal state または永続化された authority の更新を同じ completion とみなさない。`join` または task 回収という語は、対象の `JoinHandle`、`JoinSet` その他の task handle を await する経路を観測した場合だけ使う。親の future が返ることや固定の success 文字列を、spawned task の join と説明しない。両者の一致を担う writer と失敗契約が観測できなければ、task の join が保証できるのは task の終了を回収してから親が返ることだけであり、「全 job の完了後に返る」「completion の受入条件を満たす」と書かない。Accepted な契約が persisted state を authority とする場合、全 record の terminal state を確認できて初めて domain completion と判定する。
...
project の決定の記録は、README に宣言された置き場と、source、caller、受入語、authority 型から得た固有の語を手掛かりに探す。repository の外にある置き場には session で利用できる対応した読取手段を使い、直接リンクされた記録も追う。置き場の宣言や読取権限がなければ、その不足を Unknown とし、repository 内の慣例的な directory へ置き換えない。検索結果が空でも契約の不存在とはせず、宣言された置き場や直接リンクの範囲で探索を広げ、記録の採否、対象、決定内容を読む。採否は記録が使う状態表現で判定し、特定の文字列に一致することだけを条件にしない。
既存 test は内部実装名でなく、対象 source を呼ぶ一段上の公開 caller の exact symbol を手掛かりに、manifest または明示契約から確認した test root で探す。最初の search に一致がなくても test 契約の不存在とはせず、caller の rename、route、明示 link をたどって狭く再検索する。読んだ test 内で caller が返す値または状態、authority の型を検査しているかを確かめる。test root を確認できなければ慣例から推測せず Unknown にする。
設計本文を書く前に、受入条件の意味を持つ project decision record と既存 test の追跡を gate として閉じる。対象 project の宣言、source link、caller、authority、test を追っても material evidence が得られない契約は Unknown と記録し、空の search や一つの caller から不在を断定せずに設計を続ける。
...
設計だけの依頼では、変更契約を満たす構造、既存機構の使い方、観測する検証までを応答で決める。決定の記録、設計書、報告 file への記録を依頼されていなければ file を作らず、実装も編集しない。未観測の契約に依存する分岐や、隣接する migration の設計へ広げない。
