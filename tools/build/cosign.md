...
判断基準は、成果物と attestation Statement の subject digest、Build Provenance の builder identity、build type、external parameters を検証時に照合でき、署名の記録が公開の transparency log へ出ないことであり、鍵ペア署名では `--tlog-upload=false` で transparency log への送出を止める。
