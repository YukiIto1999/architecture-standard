...
台帳の各行は、規律の検証手段(機械・機械+レビュー・レビュー)とその根拠を添える。
完了条件に機械で判定できる述語を含む規律は、区分を機械または機械+レビューにし、レビューへ据え置かない。
...
- 例は、避けたい形と望ましい形を擬似コードや構造で対比し、規律の理解に役立つときだけ加える。
  差と帰結の置き場と説明コメントの禁止は [principles](../principles/README.md) の読み方の例の規則と同じにし、言語非依存に保つ。

要求と完了条件・禁止事項の写像の規則は、principles の読み方と同じにする。
