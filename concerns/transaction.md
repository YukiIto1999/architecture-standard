...
### 根拠
同時に守る整合性を一つのパスに集めれば、その範囲だけで不変条件を守れる。
集約とトランザクション境界の一致の理由は [data](../principles/data.md) に従う。
だから境界をまたぐ整合性は、イベントを介した結果整合性に委ねる。
...
一つのトランザクションが、一つの集約だけを変更している。
複数の集約にまたがる整合性が、イベントを介した結果整合性として扱われている。
...
境界をまたぐ整合性は、イベントで結果整合性にする。
同じ集約の同時の更新は、版を条件に確定して衝突を検出する。
