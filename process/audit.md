...
7. 使われている道具を、[tools](../tools/) と [languages](../languages/) の採用と判断基準に照合する。
8. 違反は、severity・ファイルと該当箇所・対応する標準の規律とその本文の引用を添えて列挙する。

## 確認点

準拠の基準は、root の [README](../README.md) の標準の参照に従い、現在の標準本文である。
直ちに是正しない違反は、単一の基線台帳ファイルへ、規律の条文名指しと該当箇所、所有者、解消条件、検出する root check を添えて記録する。基線の行は増やさず、是正で単調に減らす。
severity の対応は、root の [README](../README.md) の判定の枠に従い、各領域の正本を再定義しない。
照合で見つけた標準本文の矛盾と、標準への改訂提案は、root の [README](../README.md) の矛盾の解決に従って報告する。
