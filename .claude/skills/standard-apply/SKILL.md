...
対象 project の記録 commit を固定した後、root `README.md` と選んだ process を最初の標準本文として読む。対象 project の exact file は開始ゲートに従って先に読んでよい。
準拠 ADR は `standard_commit:` を対象 project の `docs/decisions/` に限定した Grep で探し、root の manifest は既定名の exact path を直接読む。標準の directory link はその `README.md` を直接読む。これらの発見に Glob を使わない。Grep が失敗した場合も find、Glob、directory 一覧へ切り替えず、準拠基準を未確認にする。
