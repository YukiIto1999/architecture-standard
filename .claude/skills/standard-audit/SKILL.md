...
- **J 層の役割適合**: 全規範文について、その文が答えている問いはどの層の問いかを判定し、宿主の層の役割(root README の領域表が正本)と照合する。検出クラスは、他層の問いへの回答の混入・上位規律の再定義と概念間の重複・裁定に遡れない発明・役割からの欠落。生成の癖(参照する行の直前・直後で機構の付加なく同じ規律を言い直す、機構名を伴わない再導出、選定理由・比較・却下理由の書き込み)を検出の手がかりにする。
