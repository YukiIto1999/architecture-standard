...
生成は単一の入口(task graph の contracts 生成 task)から行い、HTTP を持つ契約は OpenAPI 経由で client と型を、HTTP を持たない契約(同一プロセス・console 出力・file format)は JSON Schema 経由で型だけを、同じ `generated/` へ出す。
