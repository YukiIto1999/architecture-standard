...
採用は、TypeScript は cucumber-js である。
判断基準は、業務語彙のシナリオを公開の interface 越しに検証でき、自身が実行系なので UI の runner と独立に検証入口から起動できることである。
...
cucumber-js は自身が実行系なので、業務語彙のシナリオを browser の runner に載せずに起動できる。
