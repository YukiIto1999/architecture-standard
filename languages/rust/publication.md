...
principles の [separation](../../principles/separation.md) が定める境界と依存の向きと、concerns の [authorization](../../concerns/authorization.md) が定める入口での評価・[security](../../concerns/security.md) が定める攻撃面の最小化を、Rust の機構で満たす。

## server

### 要求
server は axum で組み、依存は State で handler へ渡す。
router は module ごとに分けて合成し、境界の仕込みは tower の middleware で一括して積む。
認証は route の一致時にだけ走る層に置く。

### 根拠
依存を State で handler へ渡せば、依存がシグネチャに現れ、組立点だけが具象を知る。
router を module ごとに分けて合成すれば、面が変更理由ごとに分かれる。
...
### 行動
依存を State で渡し、router を module ごとに nest・merge で合成する。
...
// 依存は State、router は module ごと、認証は route の一致時だけ
...
### 要求
session は tower-sessions で扱い、OIDC は openidconnect を使い、CSRF は double-submit の検査で行う。
...
cookie で認証する BFF は CSRF にさらされるので、double-submit の検査で防ぐ。

### 完了条件
session が tower-sessions で扱われている。
...
### 要求
worker は apalis を、PostgreSQL を backend にした queue で使う。
...
### 完了条件
worker が、apalis を PostgreSQL を backend にした queue で使っている。
...
### 要求
extension が接続する core のプロセスは JSON-RPC で公開し、直列化は serde を使う。
