...
判断基準は、OS 内蔵の webview に同じ viewer を載せ、core を host の back-end に置き、採用している .NET の版を target できることである。
撤回条件は、判断基準を満たさなくなることであり、webview と OS の対応状況の変化、保守の停止、採用している .NET の版を target しなくなることを再評価のトリガーとする。
