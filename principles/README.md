...
原則のフォルダを分ける単位は、下位の層のファイルが名指して参照する単位である。
...
概要と参照を置かないのは、原則が scope の除外や隣接概念との境界を持たず、リードの相互参照だけで文脈が足りるためである。
節の順序は全原則で同じにする。
...
要求の各文は、完了条件または禁止事項の少なくとも一方に対応する観測を持つ。対応の無い要求文を置かない(参照と採用の名指しの文を除く)。
