...
プロジェクトは、準拠基準とした標準のコミットハッシュを自身の ADR に記録します。

標準の Git 履歴が整理された場合でも条文照合が成立するよう、ADR にはコミットハッシュとともに記録日を併記します。コミットが到達不能になった場合は、条文の照合により基準を復元し、現行の標準へ再ピン留めを行います。監査は、ADR に記録されたコミットの標準を基準として照合します。
