...
書式と台帳の重さは、コードと文書を AI エージェントが書き、機械検査が守る前提で選んでいる。人間が手で維持する運用には最適化していない。
標準を生成するメタ層や、実装のソースは持たない。

この標準は、project ごとのアーキテクチャ決定を置き換える。
新規構築は、この標準に従って組み立てる。
...
矛盾は黙って読み替えず、ファイルと該当箇所を添えて標準の保守へ報告する。
...
標準外を採る project は、その project の ADR に、採用理由・撤回条件・単一採用・置き換える標準規律のファイル参照・逸脱の前提となる技術的制約の実証を明記する。

### 対象の性質による既定の縮小

標準は、複数の surface か永続データを持つ system を既定とする。
...
4. 規律への違反と逸脱の報告には、照合した規律の本文の引用とファイルを添える。引用は、判定の枠が層ごとに定める照合先の本文から取る。引用を添えられない違反の照合は、行われていないと見なす。合致の報告は、規律の見出しの名指しで足りる。
...
標準の履歴は整理で書き換わりうる。記録した commit が辿れなくなっても照合が成立するよう、記録には commit と記録日を併記し、照合は条文(見出しと本文)で行う。辿れない commit を見つけた project は、条文の照合で基準を復元し、現行の標準へ再 pin する。
...
すべての規範命題は、検証手段への割当を持つ。機械で検証できる規範は structure/tests と tools の inspection が割り当て、残りは process の確認点の照合が、正本のファイルまたは領域の名指しで担う。
